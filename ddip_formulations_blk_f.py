...
import pandas as pd
from numpy import array, round, append, loadtxt, savetxt, unique, zeros, ones, insert, linspace, mean, flatnonzero as find
        ...
        for j in range(t0,t0+nblocks):
            if df_term['TON'][i] > 0:
                if j + df_term['TON'][i] <= df_term['UPTIME'][i]:
                    ...
                    model.addConstr(aux_tv[i,t0][j-t0:df_term['UPTIME'][i]-1].sum() + tv[i,max(j-t0-df_term['UPTIME'][i]+1,0):j-t0+1].sum() <= tu[i,j-t0])
        ...
        for j in range(t0,t0+nblocks):
            if df_term['TON'][i] < 0:
                if j + df_term['TON'][i] <= df_term['DOWNTIME'][i]:
                    ...
                    model.addConstr(aux_tw[i,t0][j-t0:df_term['DOWNTIME'][i]-1].sum() + tw[i,max(j-t0-df_term['DOWNTIME'][i]+1,0):j-t0+1].sum() <= 1 - tu[i,j-t0])
...
def reserv_model(model,svars,ite,nblocks,t0,NT):
    ...
    if t0 + nblocks - 1 == NT:
        for i in range(NH):
            if df_hidr['TYPE'][i] == 1:
                model.addConstr(model._vol[i,nblocks-1] >= (1-vmeta/100)*(0.01*df_hidr['V0'][i]*(df_hidr['VMAX'][i]-df_hidr['VMIN'][i])+df_hidr['VMIN'][i]), name='vol_target_'+df_hidr['NAME'][i])
    ...
    for i in range(NH):
        ...
        if temp.shape[0] > 0:
            if t0 == 1:
                model.addConstr(vol[i,0] + nwbp[i,0] - nwbn[i,0] == V0 - CTE*(turb[i,0]+vert[i,0]-afl[i]-sum(df_hidr['Q0'][list(temp)])-sum(df_hidr['S0'][list(temp)])))
                for j in range(t0+1,t0+nblocks):
                    xx1 = find(tviag[list(temp)] >= j)
                    if xx1.shape[0] > 0:
                        xx2 = 0
                        for k in range(temp.shape[0]):
                            if tviag[temp[k]] >= j:
                                ...
                                xx2 += turb[temp[k],j-t0-tviag[temp[k]]] + vert[temp[k],j-t0-tviag[temp[k]]]
                        model.addConstr(vol[i,j-t0] + nwbp[i,j-t0] - nwbn[i,j-t0] == vol[i,j-t0-1]-CTE*(turb[i,j-t0] + vert[i,j-t0] - afl[i]-xx2))
                    else:
                        xx2 = 0
                        for k in range(temp.shape[0]):
                            xx2 += turb[temp[k],j-t0-tviag[temp[k]]] + vert[temp[k],j-t0-tviag[temp[k]]]
                    ...
                    if xx1.shape[0] > 0:
                        for k in range(temp.shape[0]):
                            if tviag[temp[k]] >= j:
                                ...
                                if j - tviag[temp[k]] < t0:
                                    xx2 += aux_turb[temp[k],t0][(df_hidr['TRAVELTIME'][temp[k]]-1)-(tviag[temp[k]]-1) + (j-t0)] + aux_vert[temp[k],t0][(df_hidr['TRAVELTIME'][temp[k]]-1)-(tviag[temp[k]]-1) + (j-t0)]
                                else:
                                    xx2 += turb[temp[k],j-t0-tviag[temp[k]]] + vert[temp[k],j-t0-tviag[temp[k]]]
                    else:
                        for k in range(temp.shape[0]):
                            if j - tviag[temp[k]] < t0:
                                ...
                                xx2 += turb[temp[k],j-t0-tviag[temp[k]]] + vert[temp[k],j-t0-tviag[temp[k]]]
                    if j == t0:
                        model.addConstr(vol[i,j-t0] + nwbp[i,j-t0] - nwbn[i,j-t0] == aux_vol[i]-CTE*(turb[i,j-t0] + vert[i,j-t0] - afl[i]-xx2))
                    else:
                        model.addConstr(vol[i,j-t0] + nwbp[i,j-t0] - nwbn[i,j-t0] == vol[i,j-t0-1]-CTE*(turb[i,j-t0] + vert[i,j-t0] - afl[i]-xx2))
        else:
            if t0 == 1:
                model.addConstr(vol[i,0] + nwbp[i,0] - nwbn[i,0] == V0 - CTE*(turb[i,0] + vert[i,0] - afl[i]))
            else:
                model.addConstr(vol[i,0] + nwbp[i,0] - nwbn[i,0] == aux_vol[i] - CTE*(turb[i,0] + vert[i,0] - afl[i]))
            for j in range(t0+1,t0+nblocks):
                model.addConstr(vol[i,j-t0] + nwbp[i,j-t0] - nwbn[i,j-t0] == vol[i,j-t0-1] - CTE*(turb[i,j-t0] + vert[i,j-t0] - afl[i]))
