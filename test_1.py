...
for period in range(nperiods,0,-1):
    ...
    if 1 <= period <= nperiods-1:
        ...
        for l in range(period):
            if l == period-1:
                f2 += mu['tv'][ite][0][l,period+1]*(m_f[period]._v[0] - svars['tv'][ite][0,l+1])
            else:
                f2 += mu['tv'][ite][0][l,period+1]*(m_f[period]._aux_v[l] - svars['tv'][ite][0,l+1])
        ...
        for l in range(period):
            if l == period-1:
                f2 += mu['tv'][ite][0][l,period+1]*(m_b[period]._v[0] - svars['tv'][ite][0,l+1])
            else:
                f2 += mu['tv'][ite][0][l,period+1]*(m_b[period]._aux_v[l] - svars['tv'][ite][0,l+1])
...
while best_gap >= tolerance and ite < maxiter:
    ...
    for period in range(1,nperiods+1):
        if period > 1:
            ...
            for l in range(period-1):
                m_f[period].setAttr("RHS", m_f[period].getConstrByName("ztv%d%d"%(period,l)), svars['tv'][ite][0,l+1])
    ...
    for period in range(nperiods,0,-1):
        if period > 1:
            ...
            for l in range(period-1):
                m_b[period].setAttr("RHS", m_b[period].getConstrByName("ztv%d%d"%(period,l)), svars['tv'][ite][0,l+1])
