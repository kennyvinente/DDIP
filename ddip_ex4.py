...
for period in range(1,NT+1):
    ...
    svars['tu'][ite][:,period] = array([m_f[period]._tu[i].x[0] for i in range(NG)])
    svars['tv'][ite][:,period] = array([m_f[period]._tv[i].x[0] for i in range(NG)])
    svars['tw'][ite][:,period] = array([m_f[period]._tw[i].x[0] for i in range(NG)])
    svars['gt'][ite][:,period] = array([m_f[period]._gt[i].x[0] for i in range(NG)])
    svars['vol'][ite][:,period] = array([m_f[period]._vol[i].x[0] for i in range(NH)])
    svars['turb'][ite][:,period] = array([m_f[period]._turb[i].x[0] for i in range(NH)])
    svars['vert'][ite][:,period] = array([m_f[period]._vert[i].x[0] for i in range(NH)])

    Cost_f[ite,period] = m_f[period].objval - m_f[period]._Theta[0].x
# # # # # End forward pass

UB = sum(Cost_f[ite,p] for p in range(1,NT+1));
...
tolerance = 0.001; # Percentage tolerance stopping criteria
