...
from idx_all import H_BUS, H_STATUS
...
def ext2int(ppc, val_or_field=None, ordering=None, dim=0):
    ...
    if val_or_field is None:  # nargin == 1
        ...
        if first or ppc["order"]["state"] == 'e':
            ...
            nb = ppc["bus"].shape[0]
            ng = ppc["gen"].shape[0]
            nh = ppc["hidrogen"].shape[0]
            ng0 = ng
            nh0 = nh
            if 'A' in ppc:
                dc = True if ppc["A"].shape[1] < (2 * nb + 2 * ng + 2 * nh) else False
            elif 'N' in ppc:
                dc = True if ppc["N"].shape[1] < (2 * nb + 2 * ng + 2 * nh) else False
            else:
                dc = False
            ...
            o["ext"]["hidrogen"] = ppc["hidrogen"].copy()
            if 'areas' in ppc:
            ...
            gs = ( (ppc["gen"][:, GEN_STATUS] > 0) &          ## gen status
                    bs[ (n2i[ppc["gen"][:, GEN_BUS].astype(int)]).astype(int) ] )
            hs = ( (ppc["hidrogen"][:, H_STATUS] > 0) &          ## hidrogen status
                    bs[ (n2i[ppc["hidrogen"][:, H_BUS].astype(int)]).astype(int) ] )
            o["gen"]["status"]["on"]  = find(  gs )    ## on and connected
            o["gen"]["status"]["off"] = find( ~gs )    ## off or isolated
            o["hidrogen"]["status"]["on"] = find(  hs )    ## on and connected
            o["hidrogen"]["status"]["off"] = find( ~hs )   ## off or isolated
            brs = ( ppc["branch"][:, BR_STATUS].astype(int) &  ## branch status
                    bs[(n2i[ppc["branch"][:, F_BUS].astype(int)]).astype(int)] &
                    bs[(n2i[ppc["branch"][:, T_BUS].astype(int)]).astype(int)] ).astype(bool)
...
#                ppc["gen"][o["gen"]["status"]["off"], :] = array([])
            ...
            if len(o["hidrogen"]["status"]["off"]) > 0:
                ppc["hidrogen"] = ppc["hidrogen"][o["hidrogen"]["status"]["on"], :]
...
#                ppc["areas"][o["areas"]["status"]["off"], :] = array([])
            ...
            ppc["hidrogen"][:, H_BUS] = \
                o["bus"]["e2i"][ ppc["hidrogen"][:, H_BUS].astype(int) ].copy()
            ppc["branch"][:, F_BUS] = \
            ...
            o["hidrogen"]["e2i"] = argsort(ppc["hidrogen"][:, GEN_BUS])
            o["hidrogen"]["i2e"] = argsort(o["hidrogen"]["e2i"])

            ppc["gen"] = ppc["gen"][o["gen"]["e2i"].astype(int), :]
            ppc["hidrogen"] = ppc["hidrogen"][o["hidrogen"]["e2i"].astype(int), :]
