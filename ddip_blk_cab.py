...
while best_gap >= tolerance and ite < maxiter:
    ...
    for period in range(1,NT+1,nblocks):
        if 1 <= period <= NT-nblocks:
            ...
            for i in range(NG):
                f1 += mu['tu'][ite-1][period+nblocks][i]*(m_f[period]._tu[i,nblocks-1] - svars['tu'][ite-1][i,period+nblocks-1])

                if df_term['UPTIME'][i] > 1:
                    c1,c2 = 0,1
                    for l in range(df_term['UPTIME'][i]-1,0,-1):
                        if period + nblocks - c1 - 1 <= 0:
                            break
                        if c1 < nblocks:
                            f2 += mu['tv'][ite-1][i][l-1,period+nblocks]*(m_f[period]._tv[i,nblocks - c1 - 1] - svars['tv'][ite-1][i,period + nblocks - c1 - 1])
                        else:
                            f2 += mu['tv'][ite-1][i][l-1,period+nblocks]*(m_f[period]._aux_tv[i,period][df_term['UPTIME'][i]-1-c2] - svars['tv'][ite-1][i,period + nblocks - c1 - 1])
                            c2+=1
                        c1 += 1
                ...
                if df_term['DOWNTIME'][i] > 1:
                    ...
                    for l in range(df_term['DOWNTIME'][i]-1,0,-1):
                        if period+nblocks-c1-1 <= 0:
                            break
                        if c1 < nblocks:
                            f3 += mu['tw'][ite-1][i][l-1,period+nblocks]*(m_f[period]._tw[i,nblocks-c1-1] - svars['tw'][ite-1][i,period+nblocks-c1-1])
                        else:
                            f3 += mu['tw'][ite-1][i][l-1,period+nblocks]*(m_f[period]._aux_tw[i,period][df_term['DOWNTIME'][i]-1-c2] - svars['tw'][ite-1][i,period+nblocks-c1-1])
                            c2+=1
                        c1 += 1

                f4 += mu['gt'][ite-1][period+nblocks][i]*(m_f[period]._gt[i,nblocks-1] - svars['gt'][ite-1][i,period+nblocks-1])

            for i in range(NH):
                f5 += mu['vol'][ite-1][period+nblocks][i]*(m_f[period]._vol[i,nblocks-1] - svars['vol'][ite-1][i,period+nblocks-1])
                ...
                if df_hidr['TRAVELTIME'][i] > 0:
                    ...
                    for l in range(df_hidr['TRAVELTIME'][i],0,-1):
                        if period+nblocks-c1-1 <= 0:
                            break
                        if c1 < nblocks:
                            f6 += mu['turb'][ite-1][i][l-1,period+nblocks]*(m_f[period]._turb[i,nblocks-c1-1] - svars['turb'][ite-1][i,period+nblocks-c1-1])
                            f6 += mu['vert'][ite-1][i][l-1,period+nblocks]*(m_f[period]._vert[i,nblocks-c1-1] - svars['vert'][ite-1][i,period+nblocks-c1-1])
                        else:
                            f6 += mu['turb'][ite-1][i][l-1,period+nblocks]*(m_f[period]._aux_turb[i,period][df_hidr['TRAVELTIME'][i]-1-c2] - svars['turb'][ite-1][i,period+nblocks-c1-1])
                            f6 += mu['vert'][ite-1][i][l-1,period+nblocks]*(m_f[period]._aux_vert[i,period][df_hidr['TRAVELTIME'][i]-1-c2] - svars['vert'][ite-1][i,period+nblocks-c1-1])
                            c2 += 1
                        c1 += 1

            m_f[period].addConstr(m_f[period]._Theta[0] >= obj[ite-1,period+nblocks] + f1 + f2 + f3 + f4 + f5 + f6) # Cut added to forward problem

            f1,f2,f3,f4,f5,f6 = 0,0,0,0,0,0
            for i in range(NG):
                f1 += mu['tu'][ite-1][period+nblocks][i]*(m_b[period]._tu[i,nblocks-1] - svars['tu'][ite-1][i,period+nblocks-1])
                if df_term['UPTIME'][i] > 1:
                    c1,c2 = 0,1
                    for l in range(df_term['UPTIME'][i]-1,0,-1):
                        if period + nblocks - c1 - 1 <= 0:
                            break
                        if c1 < nblocks:
                            f2 += mu['tv'][ite-1][i][l-1,period+nblocks]*(m_b[period]._tv[i,nblocks - c1 - 1] - svars['tv'][ite-1][i,period + nblocks - c1 - 1])
                        else:
                            f2 += mu['tv'][ite-1][i][l-1,period+nblocks]*(m_b[period]._aux_tv[i,period][df_term['UPTIME'][i]-1-c2] - svars['tv'][ite-1][i,period + nblocks - c1 - 1])
                            c2+=1
                        c1 += 1
                if df_term['DOWNTIME'][i] > 1:
                    c1,c2 = 0,1
                    for l in range(df_term['DOWNTIME'][i]-1,0,-1):
                        if period+nblocks-c1-1 <= 0:
                            break
                        if c1 < nblocks:
                            f3 += mu['tw'][ite-1][i][l-1,period+nblocks]*(m_b[period]._tw[i,nblocks-c1-1] - svars['tw'][ite-1][i,period+nblocks-c1-1])
                        else:
                            f3 += mu['tw'][ite-1][i][l-1,period+nblocks]*(m_b[period]._aux_tw[i,period][df_term['DOWNTIME'][i]-1-c2] - svars['tw'][ite-1][i,period+nblocks-c1-1])
                            c2+=1
                        c1 += 1
                f4 += mu['gt'][ite-1][period+nblocks][i]*(m_b[period]._gt[i,nblocks-1] - svars['gt'][ite-1][i,period+nblocks-1])

            for i in range(NH):
                f5 += mu['vol'][ite-1][period+nblocks][i]*(m_b[period]._vol[i,nblocks-1] - svars['vol'][ite-1][i,period+nblocks-1])
                ...
                if df_hidr['TRAVELTIME'][i] > 0:
                    ...
                    for l in range(df_hidr['TRAVELTIME'][i],0,-1):
                        if period+nblocks-c1-1 <= 0:
                            break
                        if c1 < nblocks:
                            f6 += mu['turb'][ite-1][i][l-1,period+nblocks]*(m_b[period]._turb[i,nblocks-c1-1] - svars['turb'][ite-1][i,period+nblocks-c1-1])
                            f6 += mu['vert'][ite-1][i][l-1,period+nblocks]*(m_b[period]._vert[i,nblocks-c1-1] - svars['vert'][ite-1][i,period+nblocks-c1-1])
                        else:
                            f6 += mu['turb'][ite-1][i][l-1,period+nblocks]*(m_b[period]._aux_turb[i,period][df_hidr['TRAVELTIME'][i]-1-c2] - svars['turb'][ite-1][i,period+nblocks-c1-1])
                            f6 += mu['vert'][ite-1][i][l-1,period+nblocks]*(m_b[period]._aux_vert[i,period][df_hidr['TRAVELTIME'][i]-1-c2] - svars['vert'][ite-1][i,period+nblocks-c1-1])
                            c2 += 1
                        c1 += 1

            m_b[period].addConstr(m_b[period]._Theta[0] >= obj[ite-1,period+nblocks] + f1 + f2 + f3 + f4 + f5 + f6) # Cut added to backward problem
...
fig_title = 'MIPGAP: ' + str(1) + '%; Runtime: ' + str(round(tet,2)) + 's; CAB; Gap: ' + str(round(best_gap,2)) + '%'
...
plt.savefig('figures/fig26.png', dpi=100)
plt.close()